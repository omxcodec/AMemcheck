...
symbol_table_t* load_symbol_table(const char *filename) {
    ...
    __libc_format_log(TANGMAI_LOG_DEBUG, "libc", "Loading symbol table from '%s'.", filename);
    ...
    int dynsym_idx = -1;
	Elf32_Half ei;
    for (ei = 0; ei < hdr->e_shnum; ei++) {
        if (shdr[ei].sh_type == SHT_SYMTAB) {
            sym_idx = ei;
        }
        if (shdr[ei].sh_type == SHT_DYNSYM) {
            dynsym_idx = ei;
        }
    }
    ...
    table = dlmalloc(sizeof(symbol_table_t));
    if(!table) {
        goto out_unmap;
    }
    table->num_symbols = 0;
    ...
    table->symbols = dlmalloc(table->num_symbols * sizeof(symbol_t));
    if (!table->symbols) {
        dlfree(table);
        table = NULL;
        goto out_unmap;
    ...
    if (dynsym_idx != -1) {
        ...
        for (i = 0; i < dynnumsyms; i++) {
            if (dynsyms[i].st_shndx != SHN_UNDEF) {
                ...
                __libc_format_log(TANGMAI_LOG_DEBUG, "libc", "  [%d] '%s' 0x%08x-0x%08x (DYNAMIC SYMBOL TABLE)",
    ...
    if (sym_idx != -1) {
        ...
        for (i = 0; i < numsyms; i++) {
            if (syms[i].st_shndx != SHN_UNDEF
                ...
                __libc_format_log(TANGMAI_LOG_DEBUG, "libc", "  [%d] '%s' 0x%08x-0x%08x (SYMBOL TABLE)",
                        symbol_index, table->symbols[symbol_index].name,
                        table->symbols[symbol_index].start, table->symbols[symbol_index].end);
                symbol_index += 1;
            }
